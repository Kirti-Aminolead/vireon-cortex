...
def generate_report_pdf(df, kpis, report_type, shed_label):
    ...
    if avg_pf > 0:
        if avg_pf < PF_PENALTY_THRESHOLD:
            ...
            if pf_rebate > 0:
                ...
                savings_items.append(['✅ PF Rebate Earned', f"₹{pf_rebate:,}/mo", f"PF {avg_pf:.2f} > 0.95"])
        else:
    ...
    if max_demand_recorded > 0 and contracted_demand > 0:
        ...
        if md_util > 90:
            savings_items.append(['⚠️ Max Demand', 'Alert', f"{max_demand_recorded:.0f}kW ({md_util:.0f}%)"])
        else:
    ...
    if fire_critical > 0:
        ...
        savings_items.append(['⚠️ Fire Safety', 'Warning', 'Inspection recommended'])
    else:
    ...
    savings_summary = [
        ...
        [f'This Period ({num_days} days)', f'₹{period_total:,}'],
        ['Monthly', f'₹{total_benefit:,}'],
        ['Annual Projection', f'₹{annual_total:,}'],
    ]
    ...
    if total_benefit == 0 and total_savings == 0:
        ...
        with s_col2:
            st.metric(
                ...
                delta=f"₹{monthly_rebate:,} rebate" if monthly_rebate > 0 else None,
                delta_color="normal"
            )
        with s_col3:
            st.metric(
                label="📊 Annual Projection",
                value=f"₹{annual_total:,}"
            )
