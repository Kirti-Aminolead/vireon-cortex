...
def generate_report_csv(df, kpis, report_type, shed_label):
    ...
    summary_data = {
        'Metric': [
            'Report Type', 'Location', 'Date Range', 'Total Readings',
            'Total Energy (kWh)', 'Peak Demand (kW)', 'Max Demand MD (kW)',
            'Avg Power Factor', 'Min Power Factor', 'PF Below 0.92 (%)',
            'Avg Load Utilization (%)', 'Max Load (%)', 'Idle Time (%)',
            'Voltage Unbalance Avg (%)', 'Current Unbalance Avg (%)',
            'Neutral Current Avg (A)', 'Neutral Current Max (A)',
            'Fire Risk - Normal', 'Fire Risk - Warning', 'Fire Risk - High', 'Fire Risk - Critical',
    ...
    # Combine into CSV
    output = StringIO()
    output.write("VIREON CORTEX - EXECUTIVE REPORT\n")
...
def main():
    ...
    # Load full data for health check (before shed filter)
    df_health = load_data_from_public_sheet(sheet_id)
