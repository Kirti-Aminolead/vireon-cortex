...
- Asset Health & Predictive Maintenance (with Pi Health monitoring)
- Risk Shield & Safety
...
Author: Kirti Ranjan, Aminobots
Version: 3.1 (v9) - WBSEDCL Tariff Compliance Update

Changes in v3.1:
- ToD period comments corrected (17:00-23:00 peak, 23:00-06:00 off-peak)
- Power Quality uses 99th percentile (excludes anomalies)
- Motor Health renamed to Phase Balance with 15% threshold
- PF Rebate/Surcharge card with ToD-wise breakdown per WBSEDCL slabs
- Load Factor calculation and rebate (WBSEDCL Up to 33kV)
- MD Analysis with Day/Night thresholds (100%/150% of CD)
- Billing MD with 85% floor rule
- Savings Banner overhauled with WBSEDCL-compliant calculations
"""
...
# Debug: Show current theme in sidebar (helps troubleshoot mobile issues)
st.sidebar.caption(f"Active theme: **{theme_choice}**")
...
<style>
    ...
    /* ============================================= */
    /* LIGHT MODE: FORCE DARK TEXT ON ALL ELEMENTS  */
    /* ============================================= */
    
    /* Mark the body as light theme for debugging */
    body::before {
        content: "LIGHT THEME";
        position: fixed;
        bottom: 5px;
    ...
    .stApp > div, .stApp .main, .stApp .block-container {
        color: #1a1a2e !important;
    }
    
    /* All Streamlit markdown and text */
    .stApp .stMarkdown, .stApp .stMarkdown p, .stApp .stMarkdown span,
    .stApp .stText, .stApp .element-container {
        color: #1a1a2e !important;
    }
    ...
    /* Streamlit selectbox/input labels */
    .stApp .stSelectbox label, .stApp .stTextInput label,
    .stApp .stDateInput label, .stApp .stNumberInput label {
        color: #4a5568 !important;
    }
    ...
    @media (max-width: 768px) {
        .stApp, .stApp *, .stApp p, .stApp span, .stApp div, .stApp label {
            color: #1a1a2e;
        }
        .stApp [data-testid="stMetricValue"],
        .stApp [data-testid="stMetricLabel"],
    ...
    @media (max-width: 768px) {
        .main-header { font-size: 24px; }
        .summary-value { font-size: 20px; color: #1a1a2e !important; }
        .summary-card { padding: 14px; }
        .kpi-card { padding: 14px; }
        .kpi-title { color: #1a1a2e !important; }
        .kpi-value { color: #1a1a2e !important; }
        .kpi-label { color: #718096 !important; }
        .section-header { margin: 24px 0 16px 0; }
        .section-title { font-size: 16px; color: #1a1a2e !important; }
        .savings-value { font-size: 32px; }
        .risk-grid { grid-template-columns: repeat(2, 1fr); }
        .rec-card { padding: 12px; flex-direction: column; }
        .rec-priority { width: 100%; height: 3px; }
        /* Force dark text on light backgrounds for mobile */
    ...
    @media (max-width: 480px) {
        .main-header { font-size: 20px; }
        .summary-value { font-size: 18px; color: #1a1a2e !important; }
        .section-badge { font-size: 9px; padding: 3px 8px; }
        .savings-value { font-size: 28px; }
    }
    
    /* ================================================== */
    /* NUCLEAR OPTION: Override ALL text in light mode   */
    /* This ensures text is never white on mobile         */
    /* ================================================== */
    @media (max-width: 1024px) {
        ...
        .stApp [style*="color: #ef"], .stApp [style*="color: #ff"],
        .stApp [style*="color: #06"], .stApp [style*="color: #f7"],
        .stApp [style*="color:#ef"], .stApp [style*="color:#ff"],
        .stApp [style*="color:#06"], .stApp [style*="color:#f7"] {
            color: inherit !important;
        }
        /* Labels should be muted */
        .kpi-label, .summary-label, .rec-category, .hardware-label {
...
def safe_get(df, column, default=0, agg='mean'):
    ...
    Supports: mean, max, min, sum, quantile_99, quantile_95, quantile_01
    """
    try:
        ...
        elif agg == 'quantile_99': return series.quantile(0.99)
        elif agg == 'quantile_95': return series.quantile(0.95)
        elif agg == 'quantile_01': return series.quantile(0.01)
        elif agg == 'quantile_05': return series.quantile(0.05)
        return default
    except Exception:
        return default
...
def load_data_from_public_sheet(sheet_id, gid="754782201", _cache_key=None):
    ...
    try:
        ...
        numeric_cols = ['kW_Total', 'kVA_Total', 'PF_Avg', 'VLN_Avg', 'VLL_Avg', 'Current_Total',
                       ...
                       'Pi_Temp_C', 'Pi_CPU_Pct', 'Pi_Mem_Pct', 'Pi_Disk_Pct', 'Pi_Uptime_Hrs']
...
def get_pf_adjustment(pf, tod_period):
    """
    Get PF rebate/surcharge percentage based on WBSEDCL slab table.
    
    Args:
        pf: Power factor value (0.0 to 1.0)
        tod_period: 'NORMAL', 'PEAK', or 'OFFPEAK'
    
    Returns:
        float: Positive = rebate %, Negative = surcharge %
    """
    # WBSEDCL PF Adjustment Slab Table
    # (pf_min, pf_max): (normal_%, peak_%, offpeak_%)
    PF_SLABS = [
        ((0.99, 1.01), (7.0, 8.0, 6.0)),    # >0.99
        ((0.98, 0.99), (6.0, 7.0, 5.0)),
        ((0.97, 0.98), (5.0, 6.0, 4.0)),
        ((0.96, 0.97), (4.0, 5.0, 0.0)),
        ((0.95, 0.96), (3.0, 4.0, 2.0)),
        ((0.80, 0.95), (0.0, 0.0, 0.0)),    # No adjustment
        ((0.00, 0.80), (-6.0, -7.0, -5.0)), # Surcharge (negative)
    ]
    
    tod_index = {'NORMAL': 0, 'PEAK': 1, 'OFFPEAK': 2}.get(tod_period.upper(), 0)
    
    for (pf_min, pf_max), adjustments in PF_SLABS:
        if pf_min <= pf < pf_max or (pf >= 0.99 and pf_max > 0.99):
            return adjustments[tod_index]
...
def calculate_kpis(df):
    ...
    # Voltage unbalance - use 99th percentile to exclude anomalies (power outages, meter glitches)
    kpis['v_unbalance_avg'] = safe_get(df, 'Voltage_Unbalance_Pct', 0, 'mean')
    kpis['v_unbalance_max'] = safe_get(df, 'Voltage_Unbalance_Pct', 0, 'quantile_99')  # P99 for display
    kpis['v_unbalance_max_raw'] = safe_get(df, 'Voltage_Unbalance_Pct', 0, 'max')  # Keep true max for reference
    kpis['v_unbalance_warning'] = safe_count(df, 'Voltage_Unbalance_Pct', lambda x: x > 2)
    
    # Current unbalance (Phase Balance) - threshold adjusted to 15%, use P99 to exclude outliers
    kpis['i_unbalance_avg'] = safe_get(df, 'Current_Unbalance_Pct', 0, 'mean')
    kpis['i_unbalance_max'] = safe_get(df, 'Current_Unbalance_Pct', 0, 'quantile_99')  # P99 for display
    kpis['i_unbalance_max_raw'] = safe_get(df, 'Current_Unbalance_Pct', 0, 'max')  # Keep true max
    kpis['i_unbalance_warning'] = safe_count(df, 'Current_Unbalance_Pct', lambda x: x > 15)
    ...
    # Grid forensics - use P1-P99 to exclude power outage artifacts (0 Hz, 0V)
    kpis['freq_min'] = safe_get(df, 'Frequency_Hz', 50, 'quantile_01')  # P1 excludes outages
    kpis['freq_max'] = safe_get(df, 'Frequency_Hz', 50, 'quantile_99')  # P99 excludes spikes
    kpis['freq_min_raw'] = safe_get(df, 'Frequency_Hz', 50, 'min')  # Keep raw for reference
    kpis['freq_max_raw'] = safe_get(df, 'Frequency_Hz', 50, 'max')
    kpis['vll_min'] = safe_get(df, 'VLL_Avg', 0, 'quantile_01')  # P1 excludes outages
    kpis['vll_max'] = safe_get(df, 'VLL_Avg', 0, 'quantile_99')  # P99 excludes spikes
    kpis['vll_min_raw'] = safe_get(df, 'VLL_Avg', 0, 'min')  # Keep raw for reference
    kpis['vll_max_raw'] = safe_get(df, 'VLL_Avg', 0, 'max')
    ...
    # ============= LOAD FACTOR CALCULATION (v3.1) =============
    # LF = (Total Energy) / (Peak Demand × Hours in Period) × 100
    kpis['load_factor'] = 0
    if kpis.get('total_energy', 0) > 0 and kpis.get('peak_demand', 0) > 0:
        hours_in_period = kpis.get('data_days', 1) * 24
        theoretical_max_energy = kpis['peak_demand'] * hours_in_period
        if theoretical_max_energy > 0:
            kpis['load_factor'] = (kpis['total_energy'] / theoretical_max_energy) * 100
    
    # Load Factor Rebate (WBSEDCL - Up to 33kV)
    kpis['lf_rebate_paise'] = 0
    lf = kpis['load_factor']
    if lf >= 90:
        kpis['lf_rebate_paise'] = 41
    elif lf >= 85:
        kpis['lf_rebate_paise'] = 31
    elif lf >= 80:
        kpis['lf_rebate_paise'] = 21
    elif lf >= 75:
        kpis['lf_rebate_paise'] = 16
    elif lf >= 70:
        kpis['lf_rebate_paise'] = 11
    elif lf >= 65:
        kpis['lf_rebate_paise'] = 6
    else:
        kpis['lf_rebate_paise'] = 0
    
    # Calculate monthly LF rebate amount
    kpis['lf_rebate_monthly'] = int(kpis.get('total_energy', 0) * kpis['lf_rebate_paise'] / 100 * (30 / max(kpis.get('data_days', 1), 1)))
    
    # ============= BILLING MD (85% Floor Rule) (v3.1) =============
    contracted_demand = kpis.get('contracted_demand', 200)
    actual_md = kpis.get('max_demand_recorded', 0)
    md_floor = contracted_demand * 0.85
    
    kpis['billing_md'] = max(actual_md, md_floor)
    kpis['md_floor'] = md_floor
    kpis['md_floor_active'] = actual_md < md_floor  # True if floor is being used
    
    # ============= MD ANALYSIS BY ToD (v3.1) =============
    # Day (06:00-23:00): 100% of CD threshold
    # Night (23:00-06:00): 150% of CD threshold
    kpis['md_day'] = 0
    kpis['md_night'] = 0
    kpis['md_day_pct'] = 0
    kpis['md_night_pct'] = 0
    kpis['md_violation_day'] = False
    kpis['md_violation_night'] = False
    
    if 'Max_Demand_kW' in df.columns and 'Timestamp' in df.columns:
        try:
            df_md = df.copy()
            df_md['Hour'] = df_md['Timestamp'].dt.hour
            df_md['MD'] = pd.to_numeric(df_md['Max_Demand_kW'], errors='coerce')
            
            # Day: 06:00-23:00, Night: 23:00-06:00
            day_mask = (df_md['Hour'] >= 6) & (df_md['Hour'] < 23)
            night_mask = (df_md['Hour'] >= 23) | (df_md['Hour'] < 6)
            
            if day_mask.sum() > 0:
                kpis['md_day'] = df_md.loc[day_mask, 'MD'].max()
                kpis['md_day_pct'] = (kpis['md_day'] / contracted_demand) * 100 if contracted_demand > 0 else 0
                kpis['md_violation_day'] = kpis['md_day_pct'] > 100
            
            if night_mask.sum() > 0:
                kpis['md_night'] = df_md.loc[night_mask, 'MD'].max()
                night_threshold = contracted_demand * 1.5  # 150% at night
                kpis['md_night_pct'] = (kpis['md_night'] / night_threshold) * 100 if night_threshold > 0 else 0
                kpis['md_violation_night'] = kpis['md_night_pct'] > 100
                
        except Exception:
            pass
    
    # ============= ToD-WISE POWER FACTOR (v3.1) =============
    # For WBSEDCL PF slab calculation
    kpis['pf_normal'] = 0
    kpis['pf_peak'] = 0
    kpis['pf_offpeak'] = 0
    kpis['pf_adj_normal'] = 0
    kpis['pf_adj_peak'] = 0
    kpis['pf_adj_offpeak'] = 0
    
    if 'PF_Avg' in df.columns and 'Timestamp' in df.columns:
        try:
            df_pf_tod = df.copy()
            df_pf_tod['Hour'] = df_pf_tod['Timestamp'].dt.hour
            df_pf_tod['PF_Valid'] = pd.to_numeric(df_pf_tod['PF_Avg'], errors='coerce')
            
            # Filter valid readings (Current > 1A, PF > 0)
            if 'Current_Total' in df_pf_tod.columns:
                current = pd.to_numeric(df_pf_tod['Current_Total'], errors='coerce')
                valid_mask = (current > 1.0) & (df_pf_tod['PF_Valid'] > 0)
                df_pf_tod = df_pf_tod[valid_mask]
            else:
                df_pf_tod = df_pf_tod[df_pf_tod['PF_Valid'] > 0]
            
            # Calculate ToD-wise average PF
            normal_mask = (df_pf_tod['Hour'] >= 6) & (df_pf_tod['Hour'] < 17)
            peak_mask = (df_pf_tod['Hour'] >= 17) & (df_pf_tod['Hour'] < 23)
            offpeak_mask = (df_pf_tod['Hour'] >= 23) | (df_pf_tod['Hour'] < 6)
            
            if normal_mask.sum() > 0:
                kpis['pf_normal'] = df_pf_tod.loc[normal_mask, 'PF_Valid'].mean()
                kpis['pf_adj_normal'] = get_pf_adjustment(kpis['pf_normal'], 'NORMAL')
            
            if peak_mask.sum() > 0:
                kpis['pf_peak'] = df_pf_tod.loc[peak_mask, 'PF_Valid'].mean()
                kpis['pf_adj_peak'] = get_pf_adjustment(kpis['pf_peak'], 'PEAK')
            
            if offpeak_mask.sum() > 0:
                kpis['pf_offpeak'] = df_pf_tod.loc[offpeak_mask, 'PF_Valid'].mean()
                kpis['pf_adj_offpeak'] = get_pf_adjustment(kpis['pf_offpeak'], 'OFFPEAK')
                
        except Exception:
            pass
    
    # Pi Health KPIs (v3.1)
    kpis['pi_temp_avg'] = safe_get(df, 'Pi_Temp_C', 0, 'mean')
    kpis['pi_temp_max'] = safe_get(df, 'Pi_Temp_C', 0, 'max')
    kpis['pi_cpu_avg'] = safe_get(df, 'Pi_CPU_Pct', 0, 'mean')
    kpis['pi_cpu_max'] = safe_get(df, 'Pi_CPU_Pct', 0, 'max')
    kpis['pi_mem_avg'] = safe_get(df, 'Pi_Mem_Pct', 0, 'mean')
    kpis['pi_mem_max'] = safe_get(df, 'Pi_Mem_Pct', 0, 'max')
    kpis['pi_disk_avg'] = safe_get(df, 'Pi_Disk_Pct', 0, 'mean')
    kpis['pi_disk_max'] = safe_get(df, 'Pi_Disk_Pct', 0, 'max')
    kpis['pi_uptime_max'] = safe_get(df, 'Pi_Uptime_Hrs', 0, 'max')
    
    # Pi health warning counts
    kpis['pi_temp_warnings'] = safe_count(df, 'Pi_Temp_C', lambda x: x > 60)
    kpis['pi_cpu_warnings'] = safe_count(df, 'Pi_CPU_Pct', lambda x: x > 80)
    kpis['pi_mem_warnings'] = safe_count(df, 'Pi_Mem_Pct', lambda x: x > 85)
    kpis['pi_disk_warnings'] = safe_count(df, 'Pi_Disk_Pct', lambda x: x > 90)
...
def main():
        ...
        if df_full is not None and 'Location' in df_full.columns:
            ...
            for idx, (_, row) in enumerate(latest.iterrows()):
                with shed_cols[idx]:
                    ...
                    # Get Pi health data from row
                    pi_temp = row.get('Pi_Temp_C', 0)
                    pi_cpu = row.get('Pi_CPU_Pct', 0)
                    pi_mem = row.get('Pi_Mem_Pct', 0)
                    ...
                    # Load status based on current
                    current_val = row.get('Current_Total', 0)
                    if current_val > 1:
                        load_status = "⚡ Active"
                        load_color = "#06d6a0"
                    elif mins_ago < 30:
                    ...
                    st.markdown(f"""
                        <div class="kpi-card" style="border-top: 3px solid {border_color};">
                            ...
                            <div class="kpi-metric">
                                <span class="kpi-label">Load Status</span>
                                <span class="kpi-value" style="color: {load_color};">{load_status}</span>
                            </div>
                            <div style="font-size: 10px; color: #8899a6; margin-top: 8px; padding-top: 8px; border-top: 1px solid #253040;">
                                <span style="color: {temp_color};">🖥️ {pi_temp:.0f}°C {temp_icon}</span> &nbsp;|&nbsp; 
                                CPU: {pi_cpu:.0f}% &nbsp;|&nbsp; Mem: {pi_mem:.0f}%
                            </div>
    ...
    with cols[3]:
        # v3.1: Show MD with Day/Night breakdown
        md_day = kpis.get('md_day', 0)
        md_night = kpis.get('md_night', 0)
        md_violation = kpis.get('md_violation_day', False) or kpis.get('md_violation_night', False)
        md_color = "#ef476f" if md_violation else "#06d6a0"
        card_class = "red" if md_violation else ""
        
        st.markdown(f"""
            <div class="summary-card {card_class}">
                <div class="summary-label">Max Demand (MD)</div>
                <div class="summary-value" style="color: {md_color}">{kpis['max_demand_recorded']:.1f} <span class="summary-unit">kW</span></div>
                <div class="summary-subtext">Day: {md_day:.0f}kW | Night: {md_night:.0f}kW</div>
            </div>
        """, unsafe_allow_html=True)
    ...
    with tab1:
        try:
            if 'Date' in df.columns and 'Energy_kWh' in df.columns and 'kW_Total' in df.columns:
                # Calculate daily consumption BY ToD PERIOD for stacked bar chart
                # Each bar = one day, stacked sections = Off-Peak / Normal / Peak
                
                has_tod = 'ToD_Period' in df.columns
                has_multiple_locations = 'Location' in df.columns and df['Location'].nunique() > 1
                
                # For energy calculation, use only main meter if multiple locations (avoid double-counting)
                if has_multiple_locations:
                    df_energy = df[df['Location'].str.contains('01|Shed_01', case=False, na=False)].copy()
                else:
                    df_energy = df.copy()
                ...
                if has_tod:
                    ...
                    df_energy['ToD_Normalized'] = df_energy['ToD_Period'].str.upper().str.replace('-', '').str.strip()
                    
                    # Group by Date AND ToD Period
                    for (date, tod), group in df_energy.groupby([pd.to_datetime(df_energy['Date']).dt.date, 'ToD_Normalized']):
                    ...
                    daily_tod = pd.DataFrame(daily_tod_stats)
                    
                    # Also calculate daily totals for metrics
                    daily = daily_tod.groupby('Date').agg({
                        'Energy_kWh': 'sum',
                        'Peak_kW': 'max'
                    }).reset_index()
                else:
                    # No ToD data - fall back to simple daily totals
                    daily_stats = []
                    for date, group in df_energy.groupby(pd.to_datetime(df_energy['Date']).dt.date):
                        group_sorted = group.sort_values('Timestamp')
                        if len(group_sorted) >= 2:
                            first_energy = group_sorted['Energy_kWh'].iloc[0]
                            last_energy = group_sorted['Energy_kWh'].iloc[-1]
                ...
                # ===== STACKED BAR CHART WITH ToD BREAKDOWN =====
                if has_tod and len(daily_tod_stats) > 0:
                    # Prepare data for stacked bar
                    daily_tod['Date'] = pd.to_datetime(daily_tod['Date'])
                    daily_tod = daily_tod[daily_tod['Energy_kWh'] < 5000]  # Filter outliers per period
                    
                    # Map ToD periods to display names
                    tod_display_map = {
                        'OFFPEAK': '🌙 Off-Peak (23:00-06:00)',
                        'NORMAL': '☀️ Normal (06:00-17:00)',
                        'PEAK': '🔥 Peak (17:00-23:00)'
                    }
                    daily_tod['ToD_Display'] = daily_tod['ToD_Period'].map(tod_display_map).fillna('Unknown')
                    
                    # Pivot for stacked chart
                    pivot_data = daily_tod.pivot_table(
                        index='Date', 
                        columns='ToD_Period', 
                        values='Energy_kWh', 
                        aggfunc='sum',
                    ...
                    for period in ['OFFPEAK', 'NORMAL', 'PEAK']:
                        if period not in pivot_data.columns:
                            pivot_data[period] = 0
                    
                    # Melt back for plotly
                    chart_data = pivot_data.melt(
                        id_vars=['Date'],
                        value_vars=['OFFPEAK', 'NORMAL', 'PEAK'],
                        var_name='ToD_Period',
                        value_name='Energy_kWh'
                    )
                    chart_data['ToD_Display'] = chart_data['ToD_Period'].map(tod_display_map)
                    
                    # Add day info for hover
                    chart_data['DayName'] = chart_data['Date'].dt.strftime('%a')
                    
                    # Define colors matching the ToD theme
                    tod_colors = {
                        '🌙 Off-Peak (23:00-06:00)': '#118ab2',  # Blue
                        '☀️ Normal (06:00-17:00)': '#ffd166',    # Yellow
                        '🔥 Peak (17:00-23:00)': '#ef476f'       # Red
                    }
                    ...
                    fig = px.bar(
                        ...
                        title='Daily Energy Consumption by Time-of-Day',
                        hover_data={'DayName': True, 'Energy_kWh': ':.1f'},
                        category_orders={'ToD_Display': [
                            '🌙 Off-Peak (23:00-06:00)',
                            '☀️ Normal (06:00-17:00)', 
                            '🔥 Peak (17:00-23:00)'
                        ]}
                    )
                    ...
                    # Show ToD breakdown metrics
                    col1, col2, col3, col4 = st.columns(4)
                    
                    total_offpeak = daily_tod[daily_tod['ToD_Period'] == 'OFFPEAK']['Energy_kWh'].sum()
                    total_normal = daily_tod[daily_tod['ToD_Period'] == 'NORMAL']['Energy_kWh'].sum()
                    total_peak = daily_tod[daily_tod['ToD_Period'] == 'PEAK']['Energy_kWh'].sum()
                    total_all = total_offpeak + total_normal + total_peak
                    
                    with col1:
                        pct = (total_offpeak / total_all * 100) if total_all > 0 else 0
                        st.metric("🌙 Off-Peak", f"{total_offpeak:,.0f} kWh", f"{pct:.0f}%")
                    with col2:
                        pct = (total_normal / total_all * 100) if total_all > 0 else 0
                        st.metric("☀️ Normal", f"{total_normal:,.0f} kWh", f"{pct:.0f}%")
                    with col3:
                        pct = (total_peak / total_all * 100) if total_all > 0 else 0
                        st.metric("🔥 Peak", f"{total_peak:,.0f} kWh", f"{pct:.0f}%")
                    with col4:
                        # Calculate cost
                        total_cost = total_offpeak * 5.18 + total_normal * 6.87 + total_peak * 8.37
                        st.metric("💰 Total Cost", f"₹{total_cost:,.0f}")
                    
                else:
                    # Fallback: Simple daily chart (no ToD data)
                    ...
                    st.info("💡 ToD breakdown not available. Add ToD_Period column to your data for shift-wise analysis.")
                
                # Show estimated gaps info if enabled
                if show_estimates and len(missing_dates) > 0:
                    estimated_total = len(missing_dates) * daily['Energy_kWh'].mean()
                ...
                if len(tod_df) > 0:
                    ...
                    if total_energy > 0:
                        ...
                        tariff_df = pd.DataFrame({
                            ...
                            'Rate (₹/kWh)': [5.18, 6.87, 8.37],
                            'Target %': ['45%', '45%', '10%'],
                            'Strategy': ['Maximize - Cheapest', 'Balance', 'Minimize - Costliest']
                        })
                        st.dataframe(tariff_df, use_container_width=True, hide_index=True)
                        
                        # ===== WHAT-IF SIMULATOR (v3.1) =====
                        st.markdown("---")
                        st.markdown("##### 🔄 What-If Load Shift Simulator")
                        st.caption("Simulate shifting load from Peak to Off-Peak and see real-time cost impact")
                        
                        # Get current energy distribution
                        current_peak = latest_data.get('PEAK', 0)
                        current_normal = latest_data.get('NORMAL', 0)
                        current_offpeak = latest_data.get('OFFPEAK', 0)
                        current_total = current_peak + current_normal + current_offpeak
                        
                        if current_total > 0 and current_peak > 0:
                            # Simulator controls
                            sim_col1, sim_col2 = st.columns([1, 1])
                            
                            with sim_col1:
                                # Slider to shift Peak → Off-Peak
                                max_shiftable = min(current_peak, current_peak * 0.8)  # Max 80% of peak can shift
                                shift_amount = st.slider(
                                    "Shift from Peak → Off-Peak (kWh)",
                                    min_value=0.0,
                                    max_value=float(max_shiftable),
                                    value=0.0,
                                    step=5.0,
                                    key="peak_to_offpeak_shift",
                                    help="Move this much energy from Peak (₹8.37/kWh) to Off-Peak (₹5.18/kWh)"
                                )
                                
                                # Optional: Shift Normal → Off-Peak
                                max_normal_shift = min(current_normal, current_normal * 0.3)  # Max 30% of normal
                                shift_normal = st.slider(
                                    "Shift from Normal → Off-Peak (kWh)",
                                    min_value=0.0,
                                    max_value=float(max_normal_shift),
                                    value=0.0,
                                    step=5.0,
                                    key="normal_to_offpeak_shift",
                                    help="Move this much energy from Normal (₹6.87/kWh) to Off-Peak (₹5.18/kWh)"
                                )
                            
                            # Calculate simulated distribution
                            sim_peak = current_peak - shift_amount
                            sim_normal = current_normal - shift_normal
                            sim_offpeak = current_offpeak + shift_amount + shift_normal
                            
                            # Calculate costs - WBSEDCL rates
                            RATE_PEAK_SIM = 8.37
                            RATE_NORMAL_SIM = 6.87
                            RATE_OFFPEAK_SIM = 5.18
                            
                            current_cost = (current_peak * RATE_PEAK_SIM + 
                                          current_normal * RATE_NORMAL_SIM + 
                                          current_offpeak * RATE_OFFPEAK_SIM)
                            
                            sim_cost = (sim_peak * RATE_PEAK_SIM + 
                                       sim_normal * RATE_NORMAL_SIM + 
                                       sim_offpeak * RATE_OFFPEAK_SIM)
                            
                            savings_sim = current_cost - sim_cost
                            savings_pct = (savings_sim / current_cost * 100) if current_cost > 0 else 0
                            
                            with sim_col2:
                                # Show savings impact
                                if savings_sim > 0:
                                    st.markdown(f"""
                                    <div style="background: rgba(6, 214, 160, 0.15); border: 1px solid #06d6a0; border-radius: 8px; padding: 16px; text-align: center;">
                                        <div style="font-size: 12px; color: #8899a6; margin-bottom: 4px;">💰 {view_mode} Savings</div>
                                        <div style="font-size: 28px; font-weight: 700; color: #06d6a0;">₹{savings_sim:,.0f}</div>
                                        <div style="font-size: 11px; color: #06d6a0;">({savings_pct:.1f}% reduction)</div>
                                        <div style="font-size: 11px; color: #8899a6; margin-top: 8px;">
                                            Monthly: ₹{savings_sim * (4 if view_mode == 'Weekly' else 1):,.0f} | Annual: ₹{savings_sim * (52 if view_mode == 'Weekly' else 12):,.0f}
                                        </div>
                                    </div>
                                    """, unsafe_allow_html=True)
                                else:
                                    st.markdown(f"""
                                    <div style="background: rgba(255, 209, 102, 0.15); border: 1px solid #ffd166; border-radius: 8px; padding: 16px; text-align: center;">
                                        <div style="font-size: 12px; color: #8899a6; margin-bottom: 4px;">📊 Current Status</div>
                                        <div style="font-size: 20px; font-weight: 600; color: #ffd166;">No shift selected</div>
                                        <div style="font-size: 11px; color: #8899a6;">Use sliders to simulate load shifting</div>
                                    </div>
                                    """, unsafe_allow_html=True)
                            ...
                            with comp_col1:
                                ...
                                | Period | Energy | Cost |
                                |--------|--------|------|
                                | 🔥 Peak | {current_peak:.0f} kWh | ₹{current_peak * RATE_PEAK_SIM:,.0f} |
                                | ☀️ Normal | {current_normal:.0f} kWh | ₹{current_normal * RATE_NORMAL_SIM:,.0f} |
                                | 🌙 Off-Peak | {current_offpeak:.0f} kWh | ₹{current_offpeak * RATE_OFFPEAK_SIM:,.0f} |
                                | **Total** | **{current_total:.0f} kWh** | **₹{current_cost:,.0f}** |
                                """)
                            
                            with comp_col2:
                                st.markdown("<div style='text-align: center; padding-top: 50px; font-size: 24px;'>➡️</div>", unsafe_allow_html=True)
                            
                            with comp_col3:
                                st.markdown("**AFTER (Simulated)**")
                                # Color coding for changes
                                peak_delta = sim_peak - current_peak
                                normal_delta = sim_normal - current_normal
                                offpeak_delta = sim_offpeak - current_offpeak
                                
                                peak_indicator = f"🔻 {abs(peak_delta):.0f}" if peak_delta < 0 else ""
                                normal_indicator = f"🔻 {abs(normal_delta):.0f}" if normal_delta < 0 else ""
                                offpeak_indicator = f"🔺 {abs(offpeak_delta):.0f}" if offpeak_delta > 0 else ""
                                
                                st.markdown(f"""
                                | Period | Energy | Cost |
                                |--------|--------|------|
                                | 🔥 Peak | {sim_peak:.0f} kWh {peak_indicator} | ₹{sim_peak * RATE_PEAK_SIM:,.0f} |
                                | ☀️ Normal | {sim_normal:.0f} kWh {normal_indicator} | ₹{sim_normal * RATE_NORMAL_SIM:,.0f} |
                                | 🌙 Off-Peak | {sim_offpeak:.0f} kWh {offpeak_indicator} | ₹{sim_offpeak * RATE_OFFPEAK_SIM:,.0f} |
                                | **Total** | **{current_total:.0f} kWh** | **₹{sim_cost:,.0f}** |
                                """)
                            
                            # Visual comparison chart
                            if shift_amount > 0 or shift_normal > 0:
                                st.markdown("---")
                                
                                # Create before/after bar chart
                                comparison_data = pd.DataFrame({
                                    'Period': ['🔥 Peak', '🔥 Peak', '☀️ Normal', '☀️ Normal', '🌙 Off-Peak', '🌙 Off-Peak'],
                                    'Scenario': ['Before', 'After', 'Before', 'After', 'Before', 'After'],
                                    'Energy (kWh)': [current_peak, sim_peak, current_normal, sim_normal, current_offpeak, sim_offpeak]
                                })
                                ...
                                fig_compare = px.bar(
                                    ...
                                    color='Scenario',
                                    barmode='group',
                                    color_discrete_map={'Before': '#5c6b7a', 'After': '#06d6a0'},
                                    title='Energy Distribution: Before vs After Shift'
                                )
                                ...
                                fig_compare.update_xaxes(gridcolor='#253040')
                                fig_compare.update_yaxes(gridcolor='#253040')
                                st.plotly_chart(fig_compare, use_container_width=True)
                                
                                # Action items
                                st.markdown("##### 📋 How to Achieve This Shift")
                                actions = []
                                if shift_amount > 0:
                                    actions.append(f"• Move **{shift_amount:.0f} kWh** of operations from Peak (5PM-11PM) to Off-Peak (11PM-6AM)")
                                    actions.append(f"  - Consider: Night shift production, automated batch processes, HVAC pre-cooling")
                                if shift_normal > 0:
                                    actions.append(f"• Move **{shift_normal:.0f} kWh** of operations from Normal (6AM-5PM) to Off-Peak (11PM-6AM)")
                                    actions.append(f"  - Consider: Early morning startups, delayed non-critical loads")
                                
                                for action in actions:
                                    st.markdown(action)
                        
                        else:
                            st.info("Insufficient Peak energy data for simulation. Need Peak > 0 kWh.")
    ...
    # Power Quality (simplified from Phase Balance Monitor) - v3.1: uses P99 to exclude outliers
    with cols[0]:
        ...
        # Improved insight logic: considers both avg and warning event percentage
        warning_pct = kpis['v_unbalance_warning'] / max(kpis['total_readings'], 1)
        if kpis['v_unbalance_avg'] < 2 and warning_pct < 0.05:
            v_insight = "✓ Stable power supply."
        elif kpis['v_unbalance_avg'] < 5:
            v_insight = "⚠️ Supply instability detected."
        else:
            v_insight = "🚨 Severe voltage unbalance. Check supply."
        st.markdown(f"""
            <div class="kpi-card">
                <div class="kpi-title">
                    ⚡ Power Quality
                ...
                <div style="font-size: 10px; color: #5c6b7a; margin-bottom: 8px;">Voltage balance across R-Y-B phases</div>
                <div class="kpi-metric">
                    <span class="kpi-label">Supply Stability</span>
                    <span class="kpi-value" style="color: {v_color}">{kpis['v_unbalance_avg']:.2f}%</span>
                </div>
                <div class="kpi-metric">
                    <span class="kpi-label">Worst (P99)</span>
    ...
    # Phase Balance (renamed from Motor Health) - v3.1: threshold adjusted to 15%, uses P99
    with cols[1]:
        i_status = "status-good" if kpis['i_unbalance_avg'] < 15 else "status-warning" if kpis['i_unbalance_avg'] < 30 else "status-critical"
        i_color = "#06d6a0" if kpis['i_unbalance_avg'] < 15 else "#ffd166" if kpis['i_unbalance_avg'] < 30 else "#ef476f"
        st.markdown(f"""
            <div class="kpi-card">
                <div class="kpi-title">
                    ⚖️ Phase Balance
                ...
                <div style="font-size: 10px; color: #5c6b7a; margin-bottom: 8px;">Current distribution across phases</div>
                <div class="kpi-metric">
                    <span class="kpi-label">Current Distribution</span>
                ...
                <div class="kpi-metric">
                    <span class="kpi-label">Events >15%</span>
                ...
                <div class="kpi-insight">
                    {"✓ Load well distributed." if kpis['i_unbalance_avg'] < 15 else "⚠️ Review single-phase loads."}
                </div>
            </div>
        """, unsafe_allow_html=True)
    
    # Capacity & Load Factor - v3.1: includes Load Factor rebate info
    with cols[2]:
        ...
        # Load Factor color (higher is better for rebate)
        lf = kpis.get('load_factor', 0)
        lf_color = "#06d6a0" if lf >= 65 else "#ffd166" if lf >= 50 else "#ef476f"
        lf_rebate = kpis.get('lf_rebate_paise', 0)
        lf_rebate_monthly = kpis.get('lf_rebate_monthly', 0)
        
        if lf_rebate > 0:
            lf_insight = f"✓ LF rebate: ₹{lf_rebate_monthly:,}/mo"
        else:
            lf_insight = "💡 Improve LF to 65%+ for rebate"
        ...
        st.markdown(f"""
            <div class="kpi-card">
                ...
                <div style="font-size: 10px; color: #5c6b7a; margin-bottom: 8px;">How efficiently you use contracted demand</div>
                <div class="kpi-metric">
                    <span class="kpi-label">Avg Utilization</span>
                    <span class="kpi-value" style="color: {load_color}">{kpis['load_avg']:.1f}%</span>
                </div>
                <div class="kpi-metric">
                    <span class="kpi-label">Load Factor</span>
                    <span class="kpi-value" style="color: {lf_color}">{lf:.1f}%</span>
                </div>
                <div class="kpi-metric">
                    <span class="kpi-label">LF Rebate</span>
                    <span class="kpi-value" style="color: {'#06d6a0' if lf_rebate > 0 else '#ffd166'}">{lf_rebate} p/kWh</span>
                </div>
                <div class="kpi-bar">
                    <div class="kpi-bar-fill" style="width: {min(lf, 100)}%; background: {lf_color};"></div>
                </div>
                <div class="kpi-insight">
                    {lf_insight}
                </div>
    ...
    # Grid Quality (simplified from Grid Forensics) - v3.1: uses P1-P99 to exclude outages
    with cols[3]:
        # Use percentile values for status check (excludes power outage artifacts)
        ...
        st.markdown(f"""
            <div class="kpi-card">
                ...
                <div style="font-size: 10px; color: #5c6b7a; margin-bottom: 8px;">Incoming supply frequency & voltage</div>
                <div class="kpi-metric">
                    <span class="kpi-label">Frequency (P1-P99)</span>
                    <span class="kpi-value" style="color: {grid_color}">{kpis['freq_min']:.1f} - {kpis['freq_max']:.1f} Hz</span>
                </div>
                <div class="kpi-metric">
                    <span class="kpi-label">Voltage (P1-P99)</span>
    ...
    # VireonEdge Device (renamed from Pi Health) - v3.1
    with cols[4]:
        # Determine device health status
        pi_temp = kpis.get('pi_temp_avg', 0)
        pi_cpu = kpis.get('pi_cpu_avg', 0)
        pi_mem = kpis.get('pi_mem_avg', 0)
        pi_disk = kpis.get('pi_disk_max', 0)
        pi_uptime = kpis.get('pi_uptime_max', 0)
        
        # Temperature icon and status
        if pi_temp < 50:
            temp_icon = "❄️"
            temp_label = "Cool"
            temp_color = "#06d6a0"
        ...
        health_issues = 0
        if pi_temp > 60: health_issues += 1
        if pi_cpu > 80: health_issues += 1
        if pi_mem > 85: health_issues += 1
        if pi_disk > 90: health_issues += 1
        
        if health_issues == 0:
            pi_status = "status-good"
            health_text = "✓ System healthy"
        elif health_issues == 1:
            pi_status = "status-warning"
            health_text = "⚠️ Minor issue"
        else:
            pi_status = "status-critical"
            health_text = "🚨 Needs attention"
        
        # CPU/Mem/Disk indicators
        cpu_icon = "✓" if pi_cpu < 80 else "⚠️" if pi_cpu < 90 else "🚨"
        mem_icon = "✓" if pi_mem < 85 else "⚠️" if pi_mem < 90 else "🚨"
        disk_icon = "✓" if pi_disk < 90 else "⚠️" if pi_disk < 95 else "🚨"
        
        st.markdown(f"""
            <div class="kpi-card">
                <div class="kpi-title">
                    🖥️ VireonEdge Device
                    <span class="status-dot {pi_status}"></span>
                </div>
                <div style="font-size: 10px; color: #5c6b7a; margin-bottom: 8px;">Edge device monitoring system</div>
                <div class="kpi-metric">
                    <span class="kpi-label">Device Temp</span>
                    <span class="kpi-value" style="color: {temp_color}">{pi_temp:.1f}°C {temp_icon}</span>
                </div>
                <div class="kpi-metric">
                    <span class="kpi-label">CPU</span>
                    <span class="kpi-value">{pi_cpu:.1f}% {cpu_icon}</span>
                </div>
                <div class="kpi-metric">
                    <span class="kpi-label">Memory</span>
                    <span class="kpi-value">{pi_mem:.1f}% {mem_icon}</span>
                </div>
                <div class="kpi-metric">
                    <span class="kpi-label">Storage</span>
                    <span class="kpi-value">{pi_disk:.1f}% {disk_icon}</span>
                </div>
                <div class="kpi-bar">
                    <div class="kpi-bar-fill" style="width: {min(pi_temp, 100)}%; background: {temp_color};"></div>
                </div>
                <div class="kpi-insight">
                    {health_text} | Uptime: {pi_uptime:.0f}h
                </div>
            </div>
        """, unsafe_allow_html=True)
    ...
    with cols[0]:
        ...
        st.markdown(f"""
            <div class="kpi-card">
                ...
                <div style="font-size: 10px; color: #5c6b7a; margin-bottom: 8px;">Neutral current & wiring safety analysis</div>
                <div class="kpi-metric">
    ...
    # PF Rebate/Surcharge (v3.1) - ToD-wise breakdown per WBSEDCL tariff
    with cols[1]:
        pf_normal = kpis.get('pf_normal', 0)
        pf_peak = kpis.get('pf_peak', 0)
        pf_offpeak = kpis.get('pf_offpeak', 0)
        
        adj_normal = kpis.get('pf_adj_normal', 0)
        adj_peak = kpis.get('pf_adj_peak', 0)
        adj_offpeak = kpis.get('pf_adj_offpeak', 0)
        
        # Calculate net adjustment (weighted by energy in each period)
        total_energy = kpis.get('total_energy', 1)
        e_normal = kpis.get('energy_normal', 0)
        e_peak = kpis.get('energy_peak', 0)
        e_offpeak = kpis.get('energy_offpeak', 0)
        
        if total_energy > 0 and (e_normal + e_peak + e_offpeak) > 0:
            weighted_adj = (
                adj_normal * (e_normal / total_energy) +
                adj_peak * (e_peak / total_energy) +
                adj_offpeak * (e_offpeak / total_energy)
            )
        else:
            weighted_adj = (adj_normal + adj_peak + adj_offpeak) / 3 if (adj_normal or adj_peak or adj_offpeak) else 0
        
        # Status based on net adjustment
        if weighted_adj > 0:
            pf_status = "status-good"
            pf_color = "#06d6a0"
            status_text = f"✓ Earning {weighted_adj:.1f}% rebate"
        elif weighted_adj < 0:
            pf_status = "status-critical"
            pf_color = "#ef476f"
            status_text = f"⚠️ Paying {abs(weighted_adj):.1f}% surcharge"
        else:
            pf_status = "status-warning"
            pf_color = "#ffd166"
            status_text = "No rebate/surcharge"
        
        def pf_display(pf_val, adj):
            if pf_val == 0:
                return "N/A", "#5c6b7a"
            if adj > 0:
                return f"{pf_val:.2f} (+{adj:.0f}%)", "#06d6a0"
            elif adj < 0:
                return f"{pf_val:.2f} ({adj:.0f}%)", "#ef476f"
            else:
                return f"{pf_val:.2f}", "#ffd166"
        
        pf_n_text, pf_n_color = pf_display(pf_normal, adj_normal)
        pf_p_text, pf_p_color = pf_display(pf_peak, adj_peak)
        pf_o_text, pf_o_color = pf_display(pf_offpeak, adj_offpeak)
        ...
        st.markdown(f"""
            <div class="kpi-card">
                ...
                <div style="font-size: 10px; color: #5c6b7a; margin-bottom: 8px;">Power factor impact on billing</div>
                <div class="kpi-metric">
                    <span class="kpi-label">Normal (06-17)</span>
                    <span class="kpi-value" style="color: {pf_n_color}">{pf_n_text}</span>
                </div>
                <div class="kpi-metric">
                    <span class="kpi-label">Peak (17-23)</span>
                    <span class="kpi-value" style="color: {pf_p_color}">{pf_p_text}</span>
                </div>
                <div class="kpi-metric">
                    <span class="kpi-label">Off-Peak (23-06)</span>
                    <span class="kpi-value" style="color: {pf_o_color}">{pf_o_text}</span>
                </div>
                <div class="kpi-bar">
                    <div class="kpi-bar-fill" style="width: {min(100, abs(weighted_adj) * 10 + 20)}%; background: {pf_color};"></div>
                </div>
                <div class="kpi-insight">
                    {status_text}
                </div>
    ...
    with cols[2]:
        ...
        st.markdown(f"""
            <div class="kpi-card">
                ...
                <div style="font-size: 10px; color: #5c6b7a; margin-bottom: 8px;">Meter verification & anti-tampering</div>
                <div class="kpi-metric">
    ...
    # WBSEDCL HT Industrial Tariff Rates
    DEMAND_CHARGE = 350  # ₹/kVA/month
    RATE_PEAK = 8.37     # ₹/kWh (17:00-23:00) - WBSEDCL Peak
    RATE_NORMAL = 6.87   # ₹/kWh (06:00-17:00) - WBSEDCL Normal
    RATE_OFFPEAK = 5.18  # ₹/kWh (23:00-06:00) - WBSEDCL Off-Peak
    
    # Scale to monthly (30 days)
    monthly_factor = 30 / max(data_days, 1)
    monthly_energy = total_energy * monthly_factor if total_energy > 0 else 5000
    
    # Calculate monthly energy cost by ToD
    monthly_energy_cost = (
        (energy_peak * monthly_factor * RATE_PEAK) +
        (energy_normal * monthly_factor * RATE_NORMAL) +
        (energy_offpeak * monthly_factor * RATE_OFFPEAK)
    )
    if monthly_energy_cost == 0 and monthly_energy > 0:
        monthly_energy_cost = monthly_energy * RATE_NORMAL
    
    # ============= 1. PF REBATE/SURCHARGE (ToD-wise per WBSEDCL) =============
    pf_adj_normal = kpis.get('pf_adj_normal', 0)
    pf_adj_peak = kpis.get('pf_adj_peak', 0)
    pf_adj_offpeak = kpis.get('pf_adj_offpeak', 0)
    
    pf_rebate_normal = (energy_normal * monthly_factor * RATE_NORMAL) * (pf_adj_normal / 100) if pf_adj_normal > 0 else 0
    pf_rebate_peak = (energy_peak * monthly_factor * RATE_PEAK) * (pf_adj_peak / 100) if pf_adj_peak > 0 else 0
    pf_rebate_offpeak = (energy_offpeak * monthly_factor * RATE_OFFPEAK) * (pf_adj_offpeak / 100) if pf_adj_offpeak > 0 else 0
    
    pf_surcharge_normal = abs((energy_normal * monthly_factor * RATE_NORMAL) * (pf_adj_normal / 100)) if pf_adj_normal < 0 else 0
    pf_surcharge_peak = abs((energy_peak * monthly_factor * RATE_PEAK) * (pf_adj_peak / 100)) if pf_adj_peak < 0 else 0
    pf_surcharge_offpeak = abs((energy_offpeak * monthly_factor * RATE_OFFPEAK) * (pf_adj_offpeak / 100)) if pf_adj_offpeak < 0 else 0
    
    total_pf_rebate = pf_rebate_normal + pf_rebate_peak + pf_rebate_offpeak
    total_pf_surcharge = pf_surcharge_normal + pf_surcharge_peak + pf_surcharge_offpeak
    
    if total_pf_rebate > 0:
        savings_breakdown.append(f"✅ PF Rebate: ₹{int(total_pf_rebate):,}/mo (N:{pf_adj_normal:.0f}%, P:{pf_adj_peak:.0f}%, OP:{pf_adj_offpeak:.0f}%)")
        total_rebate += total_pf_rebate
    elif total_pf_surcharge > 0:
        savings_breakdown.append(f"⚠️ PF Surcharge: ₹{int(total_pf_surcharge):,}/mo - Improve PF above 0.80")
        total_savings += total_pf_surcharge  # Potential savings if PF improved
    else:
        savings_breakdown.append(f"⚪ PF Status: Good ({avg_pf:.2f}) - No rebate/surcharge")
    
    # ============= 2. LOAD FACTOR REBATE (WBSEDCL) =============
    lf_rebate = kpis.get('lf_rebate_monthly', 0)
    if lf_rebate > 0:
        savings_breakdown.append(f"✅ Load Factor Rebate: ₹{int(lf_rebate):,}/mo ({kpis.get('lf_rebate_paise', 0)} p/kWh at {load_factor:.0f}% LF)")
        total_rebate += lf_rebate
    elif load_factor < 65:
        potential_lf_rebate = int(monthly_energy * 6 / 100)  # Minimum rebate at 65% LF
        savings_breakdown.append(f"💡 Load Factor: {load_factor:.0f}% - Improve to 65%+ for ₹{potential_lf_rebate:,}/mo rebate")
    else:
        savings_breakdown.append(f"⚪ Load Factor: {load_factor:.0f}%")
    
    # ============= 3. DEMAND CONTRACT OPTIMIZATION =============
    if peak_demand > 0 and contracted_demand > 0:
        optimal_contract = peak_demand * 1.2  # 20% buffer
        utilization = (peak_demand / contracted_demand) * 100
        
        if optimal_contract < contracted_demand * 0.85:
            # Can reduce contract (below 85% floor)
            current_demand_cost = billing_md * DEMAND_CHARGE
            new_billing_md = max(optimal_contract, optimal_contract * 0.85)
            new_demand_cost = new_billing_md * DEMAND_CHARGE
            demand_savings = int(current_demand_cost - new_demand_cost)
            
            if demand_savings > 500:
                savings_breakdown.append(f"💰 Demand Optimization: ₹{demand_savings:,}/mo - Reduce to {optimal_contract:.0f}kW contract")
                total_savings += demand_savings
            else:
                savings_breakdown.append(f"⚪ Demand Contract: {utilization:.0f}% utilized - well-sized")
        else:
            savings_breakdown.append(f"⚪ Demand Contract: {utilization:.0f}% utilized - well-sized")
    
    # ============= 4. MD VIOLATION CHECK =============
    md_violation_day = kpis.get('md_violation_day', False)
    md_violation_night = kpis.get('md_violation_night', False)
    md_day_pct = kpis.get('md_day_pct', 0)
    md_night_pct = kpis.get('md_night_pct', 0)
    
    if md_violation_day:
        excess_md = kpis.get('md_day', 0) - contracted_demand
        penalty_estimate = int(excess_md * DEMAND_CHARGE)
        savings_breakdown.append(f"🚨 MD Violation (Day): {md_day_pct:.0f}% of CD - Est. penalty ₹{penalty_estimate:,}/mo")
    elif md_day_pct > 90:
        savings_breakdown.append(f"⚠️ MD Alert (Day): {md_day_pct:.0f}% of CD - Monitor closely")
    else:
        savings_breakdown.append(f"✅ MD (Day): {md_day_pct:.0f}% of CD - Safe")
    
    if kpis.get('md_night', 0) > 0:
        if md_violation_night:
            night_threshold = contracted_demand * 1.5
            excess_md = kpis.get('md_night', 0) - night_threshold
            penalty_estimate = int(excess_md * DEMAND_CHARGE)
            savings_breakdown.append(f"🚨 MD Violation (Night): >{md_night_pct:.0f}% of 150% CD - Est. penalty ₹{penalty_estimate:,}/mo")
        else:
            savings_breakdown.append(f"✅ MD (Night): {md_night_pct:.0f}% of 150% threshold - Safe")
    
    # ============= 5. ToD SHIFT OPTIMIZATION =============
    if energy_peak > 0:
        monthly_peak = energy_peak * monthly_factor
        rate_diff = RATE_PEAK - RATE_OFFPEAK  # 8.37 - 5.18 = 3.19
        shiftable_energy = monthly_peak * 0.3  # 30% realistically shiftable
        tod_savings = int(shiftable_energy * rate_diff)
        
        if tod_savings > 500:
            savings_breakdown.append(f"💡 ToD Shift: ₹{tod_savings:,}/mo potential - Move {shiftable_energy:.0f}kWh to off-peak")
            total_savings += tod_savings
    ...
    if total_benefit == 0 and total_savings == 0:
        ...
        st.markdown("""
            <div class="savings-banner">
                <div class="savings-label">💰 Savings & Benefits Summary (WBSEDCL Compliant)</div>
            </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
            <div style="background: rgba(6, 214, 160, 0.05); border-radius: 8px; padding: 12px; margin-top: 8px;">
                <div style="font-size: 11px; font-weight: 600; margin-bottom: 8px; opacity: 0.9; color: inherit;">📋 Detailed Breakdown:</div>
                <div style="text-align: left; font-size: 11px; line-height: 1.8; color: inherit;">{savings_text}</div>
                <div style="font-size: 9px; opacity: 0.7; margin-top: 12px; border-top: 1px solid rgba(128,128,128,0.2); padding-top: 8px; color: inherit;">
