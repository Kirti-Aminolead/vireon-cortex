...
def main():
    ...
    with st.sidebar.expander("📋 Alert Data", expanded=False):
        ...
        if alert_type == "🔥 Fire Risk Events":
            ...
            # PF sustained: 3+ consecutive readings below 0.90 with Current > 1A
        ...
        try:
            if 'ToD_Period' in df.columns:
                ...
                if len(tod_df) > 0:
                    ...
                    # Use Year-Week format to handle year boundary (Dec 2025 → Jan 2026)
                    tod_df['Week'] = tod_df['Date'].dt.strftime('%Y-W%W')
