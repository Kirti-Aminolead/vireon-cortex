...
class MeterMonitor:
    ...
    def display_readings(self, data):
        """Display readings with ToD billing info - v2.1 with new parameters"""
        print("\n" + "="*90)
        print("L&T WL4405 3-PHASE MONITOR v3.0 - {} - {}".format(
            CONFIG['device_id'], datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        print("Location: {} | Serial: {} | CT: {}/{}A".format(
            CONFIG['location'], CONFIG['meter_serial'], 
            CONFIG['ct_primary'], CONFIG['ct_secondary']))
        print("="*90)
        ...
        print("\nToD Period: {} | Base Rate: Rs.{}/kWh | PF Rebate: {}% | Effective: Rs.{:.4f}/kWh".format(
            tod_period, base_rate, pf_rebate, effective_rate))
        
        # Phase-wise display
        print("\n{:<20} {:>12} {:>12} {:>12} {:>12}".format(
            "Parameter", "R-Phase", "Y-Phase", "B-Phase", "Total/Avg"))
        print("-"*72)
        
        # Voltages L-N
        print("{:<20} {:>10.1f} V {:>10.1f} V {:>10.1f} V {:>10.1f} V".format(
            "Voltage (L-N)",
        ...
        print("{:<20} {:>10.1f} V {:>10.1f} V {:>10.1f} V {:>10.1f} V".format(
            "Voltage (L-L)",
            get_val('V_RY'), get_val('V_YB'), get_val('V_BR'), get_val('VLL_Avg')))
        
        # Currents
        print("{:<20} {:>10.2f} A {:>10.2f} A {:>10.2f} A {:>10.2f} A".format(
            "Current",
            get_val('Current_R'), get_val('Current_Y'), get_val('Current_B'),
            get_val('Current_Total')))
        
        # Active Power
        print("{:<20} {:>10.1f} W {:>10.1f} W {:>10.1f} W {:>10.1f} W".format(
            "Active Power",
            get_val('Watts_R'), get_val('Watts_Y'), get_val('Watts_B'),
            get_val('Watts_Total')))
        
        # VA
        print("{:<20} {:>9.1f} VA {:>9.1f} VA {:>9.1f} VA {:>9.1f} VA".format(
            "Apparent Power",
            get_val('VA_R'), get_val('VA_Y'), get_val('VA_B'), get_val('VA_Total')))
        
        # PF
        print("{:<20} {:>12.3f} {:>12.3f} {:>12.3f} {:>12.3f}".format(
            "Power Factor",
            get_val('PF_R'), get_val('PF_Y'), get_val('PF_B'), pf_avg))
        
        # ===== CORTEX FEATURES =====
        print("\n" + "-"*90)
        print("CORTEX FEATURES:")
        print("-"*90)
        
        # Neutral Current & Fire Risk
        neutral_current = get_val('Neutral_Current_Calc')
        fire_risk = get_val('Fire_Risk_Level')
        risk_indicator = "⚠️ " if fire_risk in ['WARNING', 'HIGH', 'CRITICAL'] else "✓ "
        print("{:<25} {:>10.2f} A   Fire Risk: {}{}".format(
            "Neutral Current (Calc)", neutral_current, risk_indicator, fire_risk))
        
        # Unbalance
        v_unbalance = get_val('Voltage_Unbalance_Pct')
        i_unbalance = get_val('Current_Unbalance_Pct')
        v_warn = "⚠️ " if v_unbalance > CONFIG['voltage_unbalance_warning'] else "✓ "
        i_warn = "⚠️ " if i_unbalance > CONFIG['current_unbalance_warning'] else "✓ "
        print("{:<25} {:>10.2f} %   {}".format("Voltage Unbalance", v_unbalance, v_warn))
        print("{:<25} {:>10.2f} %   {}".format("Current Unbalance", i_unbalance, i_warn))
        
        # Run Hours
        run_hours = get_val('Run_Hours')
        print("{:<25} {:>10.2f} hrs ({:.1f} days)".format(
            "Run Hours", run_hours, run_hours / 24))
        
        # Load
        load_pct = get_val('Load_Pct')
        print("{:<25} {:>10.2f} kW ({:.1f}% of {} kW)".format(
            "Current Load", get_val('kW_Total'), load_pct, CONFIG['contracted_load_kw']))
        print("{:<25} {:>10.2f} kVA".format("Apparent Power", get_val('kVA_Total')))
        
        # Frequency
        print("{:<25} {:>10.2f} Hz".format("Frequency", get_val('Frequency')))
        
        # Energy & Carbon
        energy_kwh = get_val('Energy_Wh') / 1000
        carbon_kg = get_val('Carbon_kg')
        print("{:<25} {:>10.2f} kWh".format("Total Energy", energy_kwh))
        print("{:<25} {:>10.2f} kg CO2".format("Carbon Footprint", carbon_kg))
        
        # ===== Today's consumption & cost =====
        today_data = self.tracker.get_today_summary()
        energy_wh = get_val('Energy_Wh')
        session = self.tracker.get_session_consumption(energy_wh)
        
        total_energy = today_data['energy_normal'] + today_data['energy_peak'] + today_data['energy_offpeak']
        total_cost = today_data['cost_normal'] + today_data['cost_peak'] + today_data['cost_offpeak']
        
        print("\n" + "="*90)
        print("TODAY'S CONSUMPTION & COST (ToD Breakdown):")
        print("-"*90)
        print("{:<20} {:>15} {:>15} {:>15} {:>15}".format(
            "Period", "Energy (kWh)", "Rate (Rs)", "Cost (Rs)", "Hours"))
        print("-"*90)
        
        # Approximate hours per period so far
        now = datetime.now()
        normal_hrs = max(0, min(now.hour, 17) - 6) if now.hour >= 6 else 0
        peak_hrs = max(0, min(now.hour, 23) - 17) if now.hour >= 17 else 0
        offpeak_hrs = now.hour if now.hour < 6 else (24 - 23 + now.hour if now.hour >= 23 else 0)
        
        print("{:<20} {:>15.2f} {:>15.2f} {:>15.2f} {:>15}".format(
            "Normal (06-17)",
            today_data['energy_normal'] / 1000,
            CONFIG['rate_normal'],
            today_data['cost_normal'],
            "{} hrs".format(normal_hrs)))
        
        print("{:<20} {:>15.2f} {:>15.2f} {:>15.2f} {:>15}".format(
            "Peak (17-23)",
            today_data['energy_peak'] / 1000,
            CONFIG['rate_peak'],
            today_data['cost_peak'],
            "{} hrs".format(peak_hrs)))
        
        print("{:<20} {:>15.2f} {:>15.2f} {:>15.2f} {:>15}".format(
            "Off-Peak (23-06)",
            today_data['energy_offpeak'] / 1000,
            CONFIG['rate_offpeak'],
            today_data['cost_offpeak'],
            "{} hrs".format(offpeak_hrs)))
        
        print("-"*90)
        print("{:<20} {:>15.2f} {:>15} {:>15.2f}".format(
            "TOTAL",
            total_energy / 1000,
            "",
            total_cost))
        ...
        # Max Demand
        print("\n{:<20} {:>15.2f} kVA | {:>15.2f} kW".format(
            "Max Demand Today",
            today_data['max_demand_kva'],
            today_data.get('max_demand_kw', 0)))
        
        # Google Sheets status
        if self.sheets_logger.enabled:
            status = "✓ Connected" if not self.sheets_logger.last_push_error else "✗ Error"
